...
import { getDragLabel, type CommonRenderArgs, type StickyBoardObject } from './boardObjectRendererShared'

export const renderStickyObject = (args: CommonRenderArgs<StickyBoardObject>): ReactElement => {
  const {
    boardObject,
    position,
    size,
  ...
  const isInlineStickyTextEditing = inlineEditor?.objectId === boardObject.id && inlineEditor.field === 'text'
    ...
    <Group
      ...
      onDragEnd={(event) => {
        void endObjectDrag(
          ...
          getDragLabel(selectedIdsCount, 'sticky'),
        )
      }}
    >
      {shapeType === 'circle' ? (
